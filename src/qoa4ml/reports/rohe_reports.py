...
class RoheReport(AbstractReport):
    ...
    def combine_stage_report(
        ...
        # StageReport instances cannot bleed back into ``self.inference_report``.
    ...
    def process_previous_report(self, previous_report_dict: dict) -> None:
        ...
        Notes
        -----
        - Raises ``ValueError`` when ``inference_report`` or
          ``execution_graph`` is missing from the input.
        - Service, data, security, and ML-specific reports plus the
          execution graph are combined with the current report.
        """
    ...
    def observe_metric(
        ...
        # Deep-copy so callers mutating their Metric afterwards can't
        # leak into the stored report.
